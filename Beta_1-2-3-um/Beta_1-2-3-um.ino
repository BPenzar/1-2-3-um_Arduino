...
uint32_t white   = leds.Color(248,180,255);
...
void positioning() {
    ...
    testwert = irRead() ;
    if(testwert > 95 && testwert < 125) {                  // Bei 5 erfolgreichen Tests gamecheck = Play
        led_farbe(1,green);
  ...
  /* Verhalten der Spieler während des Spiels prüfen. Laufen während sie nicht dürfen, Sieg, etc. */
    ...
    if ( Stop == false ) {
        for (int j = 0; j < randomtime * 2; j++) {
            ...
            if (vergleichswert > 600) {
                gamecheck = Win;
                break;
            }
            //delay (450); // ca. 1sek pro randomtime-Einheit warten
        }
    }
    
    // --- Game-LED red --- Stopphase
    else if ( Stop == true ) {
        delay(300); // Verzögert Vergleichwert für Stoppphase
        vergleichswert = irRead(); 
        ...
        for (int k=0; k < randomtime ; k++ ) {
            ...
            if (differenz > 20) { // Spieler bei unerlaubter Bewegung zurück an Startposition
                fehlerzaehler++;
                Serial.print("Fehler: ");
                Serial.println(fehlerzaehler);
            }
            if (fehlerzaehler >=2) {
              gamecheck = Lose;  
              break;
            }
            if (fehlerzaehler = 0) delay (700); // ca. 1sek pro randomtime-Einheit warten
        }
    }
